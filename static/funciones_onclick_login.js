		...
		$(document).ready(function (){
			$("#ejecutarObtenerLogin1").on('click', function(){
				$("#ejecutarObtenerLogin1").off("click"); // hago que solo se ejecute una vez la accion
				console.log('login 1');
		...
		function show_accounts(){ 
			...
			api.integration.wunderlist.listAccounts(function (error, accounts) {
				...
				if(accounts.length > 0){
					...
					wunderlist_win.on(".wz-app-235").css({'width': '464px', 'min-width': '464px', 'height': '536px', 'min-height': '536px'});
		...
		function añadirCuentaNueva(){
			$("#ejecutarObtenerLogin2").on('click', function(){
				$("#ejecutarObtenerLogin2").off("click"); // hago que solo se ejecute una vez la accion
				console.log('login 2');
		...
		function borrarCuentaSeleccioanda(){
			$(".ejecutarEliminarCuenta").on('click', function(){
				$(".ejecutarEliminarCuenta").off("click"); // hago que solo se ejecute una vez la accion
...
///////////////////////// INICIO LISTAS, NOTAS, ETC.////////////////////////////////
		...
		function mostrarList(cuentaSelect){
			...
			cuentaSelect.getLists(function (error, listas) {
				...
				//CREAR LISTA --> FUNCTION
				crearListaNueva(cuentaSelect);
				...
				$(".sidebarItem").on('click', function(e){
					...
					for (var i=1;i<=totalListas;i++){
						if ($(this).attr('id') == 'lista'+i){
							...
							//MODICICAR LISTA --> FUNCTION
							modificarListaNueva(cuentaSelect, idList, i);
						}else{
					...
					//FUNCTION -> CREAR NOTA
					crearTareaNueva(cuentaSelect, idList);
				});	
			});
		...
		// RESALTAR TAREA SELECCIONADA CON COLOR AZUL
				function tareaSeleccionada(cuentaSelect, idList){
					...
					$(".tarea-seleccionada li").on('click', function(e){
						e.stopPropagation();
...
//////////////////// FUNCIONES DE DISEÑO ////////////////////////////////////////		
			...
			// LISTA	--> CREAR  Y OCULTAR
			function crearListaNueva(cuentaSelect){
				// comprobamos que hay texto escrito
				$("#nombreCrearLista").keypress(function(){
					if($(this).val() != ''){
						// si lo hay...
						$("#ejecutarGuardarCrearLista").removeClass('disabled');
						//ejecutamos api
						$("#ejecutarGuardarCrearLista").on('click', function(){
							$("#ejecutarGuardarCrearLista").off("click"); // hago que solo se ejecute una vez la accion
							...
							cuentaSelect.createList(title, function (error, crearListaAccion) {
								...
								$("#ejecutarGuardarCrearLista").addClass('disabled');// anulamos el boton
								//actualizamos lists
								mostrarList(cuentaSelect); 
							});
						});
						...
						//si no lo hay
						$("#ejecutarGuardarCrearLista").addClass('disabled');
					}
				});
			}
			
			// LISTA	--> MODIFICAR O ELIMINAR
			function modificarListaNueva(cuentaSelect, idList, i){
				let tituloLista = $("#lista"+i).attr('title');
				// damos disable y active a modificar y eliminar lista
				if (i > 7){
					$(".ejecutarMostrarCompartir").removeClass('disabled').addClass('active');
					$("#nombreModificarEliminarLista").attr('value',tituloLista);
				}else{
					$(".ejecutarMostrarCompartir").addClass('disabled').removeClass('active');
				}
				
				//ejecutamos api modificar lista
				$("#ejecutarOcultarCompartir").on('click', function(){
					$("#ejecutarOcultarCompartir").off("click"); // hago que solo se ejecute una vez la accion
					let updateData = $("#nombreModificarEliminarLista").val();
					console.log('modificar listas');
					let idListN = parseInt(idList);
				...
				//ejecutamos api eliminar lista
				$(".confirmarEliminarLista").on('click', function(){ 
					$(".confirmarEliminarLista").off("click"); // hago que solo se ejecute una vez la accion
			...
			// TAREA	--> CREAR
			function crearTareaNueva(cuentaSelect, idList){
				// comprobamos que hay texto escrito
				$("#nombreCrearNota").keypress(function(e){
					$("#nombreCrearNota").off("click"); // hago que solo se ejecute una vez la accion
			...
			function mostrarMenuDerecho(cuentaSelect, idList){
				...
				$(".confirmarEliminarTarea").on('click', function(){ 
					$(".confirmarEliminarTarea").off("click"); // hago que solo se ejecute una vez la accion
			...
			function modificarTareaSeleccionada(cuentaSelect, idList){
				...
				$("#ejecutarModificarNombreTarea").keypress(function(e){
					$("#ejecutarModificarNombreTarea").off("click"); // hago que solo se ejecute una vez la accion
